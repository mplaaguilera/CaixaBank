({
	...
	recordDataUpdated: function(component, event) {
		if (event.getParams().changeType === 'LOADED' || event.getParams().changeType === 'CHANGED') {
			...
			// Habilitar/deshabilitar botón Pendiente Interno
			let estado = component.get('v.caso').Status;
			component.set('v.botonPendienteInternoActivo', estado !== 'Cerrado' && estado !== 'Rechazado' && estado !== 'Pendiente Colaborador' && estado !== 'Pendiente Cliente');
			...
			if (response.getState() === 'SUCCESS') {
				...
				buscarColaborador.setCallback(this, responseBuscarColaborador => {
					if (responseBuscarColaborador.getState() === 'SUCCESS') {
						...
						let args = {
							'actionName': 'Case.AM_Enviar_Correo',
							'targetFields': {
	...
	remitirColaborador: function(component) {
		...
		prepararCaso.setParams({
			'idCaso': component.get('v.recordId'),
			'plantilla': idPlantillaSeleccionada,
			'informarReferenciaCorreo': false,
			'operativa': component.get('v.tipoOperativa')
		});
		prepararCaso.setCallback(this, response => {
			if (response.getState() === 'SUCCESS') {
	...
	modalPendienteInternoAbrir: function(component) {
		component.set('v.renderModalPendienteInterno', true);
		$A.util.addClass(component.find('backdrop'), 'slds-backdrop--open');
		$A.util.addClass(component.find('modalPendienteInterno'), 'slds-fade-in-open');
		window.setTimeout($A.getCallback(() => component.find('botonPendienteInterno').focus()), 20);
	},
	modalPendienteInternoCerrar: function(component) {
		$A.util.removeClass(component.find('modalPendienteInterno'), 'slds-fade-in-open');
		$A.util.removeClass(component.find('backdrop'), 'slds-backdrop--open');
		component.set('v.renderModalPendienteInterno', false);
	},

	botonActualizarPendienteInterno: function(component, event, helper){
		//component.find('botonPendienteInterno').set('v.disabled', true);
			
			let actualizarStatus = component.get('c.actualizarCaseStatus');
			actualizarStatus.setParams({
				'idCaso': component.get('v.recordId'),
				'currentStatus': component.find('updateStatus').get('v.value'),
				'detalleSolucion':component.find('detallesSolucionPendienteInterno').get('v.value')});
			actualizarStatus.setCallback(this, response => {
				if (response.getState() === 'SUCCESS') {
					$A.enqueueAction(component.get('c.modalPendienteInternoCerrar'));
					helper.mostrarToast('success', response.getReturnValue(), '');
					helper.mostrarToast('success', 'Caso Actualizado', 'El estado del caso ha sido actualizado con éxito.');
					$A.get('e.force:refreshView').fire();
				} else if (response.getState() === 'ERROR') {
					let errors = actualizarStatus.getError();
					if (errors) {
						let mensajeError = '';
						errors.forEach(error => mensajeError += error.message + '\n');
						helper.mostrarToast('error', 'No se ha podido actualizar el caso', mensajeError);
					}
				}
			});
			$A.enqueueAction(actualizarStatus);

({
				...
				comprobarFamilia.setCallback(this, response => {
					if (response.getState() === 'SUCCESS') {
						...
						comprobarUsuarioSAC.setCallback(this, responseComprobarUsuarioSAC => {
							...
							if((!component.get('v.esPropietario') && !component.get('v.esAuditorSAC')) || (component.get('v.caso.Status') == 'Cerrado' && !component.get('v.esAuditorSAC')) || component.get('v.caso.Status') == 'Rechazado') {
								component.set('v.puedeGuardar', false);
							} else {
								component.set('v.puedeGuardar', true);
							}
						});
	...
	receiveLWCData: function (component, event, helper) {
		component.set("v.mostrarBuscador", event.getParam("dataToSend"));
	}
});
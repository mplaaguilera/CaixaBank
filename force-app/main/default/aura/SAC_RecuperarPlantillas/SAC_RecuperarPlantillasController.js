({
		...
		recuperarTemplates.setCallback(this, function(response){
			...
			if (state === "SUCCESS") { 
				...
				/*for (var miPlantilla in plantillas) {
					...
					options.push({ label: plantilla.nombrePlantilla, value: plantilla.id });
				}            

				component.set('v.optionsPlantilla', JSON.parse(JSON.stringify(options)));*/
			}
		});
	...
	handleChangePlantilla : function(component, event, helper){
		//alert('Se ha seleccionado la plantilla ' + event.getParam("value"));
	},
		...
		if(idRaiz != idPadre){
			...
			vueltaAtras.setCallback(this, function(response){
				...
				if (state === "SUCCESS") {
					...
					//alert(JSON.stringify(ruta[ultimaPosicion]));

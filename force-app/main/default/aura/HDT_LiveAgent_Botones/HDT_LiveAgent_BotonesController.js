...
({
			...
			getLiveChatTranscript.setCallback(this, function (response) {
				if (response.getState() === 'SUCCESS') {
					...
					if (resultado.CC_Aplicacion_Origen__c == 'Whatsapp') {
						...
						actionWhats.setCallback(this, response => {
							if (response.getState() === 'SUCCESS') {
				...
				var mensaje = component.get('v.mensaje');
				if (mensaje == null) {
					let action1 = component.get('c.getMensajesPausaChat');
					action1.setParams({
						areaChat: sAreaChat,
						idioma: resultado.CC_IdiomaCV__c
					});
					action1.setCallback(this, response => {
						
						if (response.getState() === 'SUCCESS') {
							mensaje = response.getReturnValue();
							component.set('v.mensaje', mensaje);
						} else {
	...
	onChangeLanguage: function (component) {
		...
		changeLanguage.setCallback(this, response => {
			...
			if (component.isValid() && state === 'SUCCESS') {
				...
				if (cognitiveCode == undefined || cognitiveCode == null) {
		...
		getMensajesChat.setCallback(this, response => {
			...
			if (list !== null) {
				if (list.length > 0) {
					...
					//codCognitive = list[0].CC_Codigo_Cognitive__c;
					//console.log('codCognitive: '+codCognitive);
	...
	onCodeSelectFirstChangeEspacio: function (component, event, helper) {
		...
		//component.set("v.actualFirstOption", actualOption);
		let picklistOptionsEspacio = component.get('v.picklistFirstOptionsEspacio');
	...
	pausar: function (cmp, evt, hlp) {
		//var mensaje = cmp.get("v.mensaje");
		//var idioma = cmp.get("v.radioValue");
		//var areaChatV = cmp.get("v.areaChat");
		...
		if (label === 'Pausar') {
			...
			hlp.reaundarChat(cmp, start, end);

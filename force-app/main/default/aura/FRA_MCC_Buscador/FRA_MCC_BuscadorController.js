({
	recordDataUpdated: function(component, event) {
		if (event.getParams().changeType === 'LOADED' || event.getParams().changeType === 'CHANGED') {
			...
			if (component.get('v.caso.RecordType.DeveloperName') != null && component.get('v.caso.RecordType.DeveloperName') != undefined) {
			...
			if (component.get('v.negocio') === 'FRA' || !component.get('v.caso.CC_MCC_Motivo__r.Name')) {
				...
				if (enteredValue.length >= 3) {
					...
					component.set('v.idTimeout', setTimeout($A.getCallback(() => {
						...
						if (component.get('v.negocio') !== 'OS') {
								...
								queryString +=' AND CC_Tipo_Cliente__c IN (\'Cliente (FRA)\') LIMIT 15'
						}

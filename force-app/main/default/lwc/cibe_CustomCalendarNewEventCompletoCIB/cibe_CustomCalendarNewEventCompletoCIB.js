...
import getEvents from '@salesforce/apex/CIBE_CustomCalendar_Controller.retrieveEventsCIB';
...
export default class Av_CustomCalendar extends LightningElement {
	...
	addEvent(date) { 
		...
		.then((data) => {
			...
			let validation = true;
			if(that.evsubject === null || that.evsubject === undefined || that.evsubject === ''){
				validation = false;
			}
	...
	changeSubjectEvent(subjectEvent) { 
		...
		that.arrEvent = [];               
		let validation = true;
		if(subjectEvent === null || subjectEvent === undefined || subjectEvent === ''){
			validation = false;
		}
		if(!that.gestoroverwrite){
			that.events.forEach(evt=>{
	...
	changeOwnerEvent(userId, gestorOverWrite ) { 
        console.log('## that.isMonth ', that.isMonth);
		const ele2 = this.template.querySelector('div.fullcalendarjs');
		$(ele2).fullCalendar('destroy');
		var initialLocaleCode = LANG === 'es' ? 'es' : 'en';
		var that = this;
		const ele = this.template.querySelector('div.fullcalendarjs');
		that.arrEvent = [];
		var moment = $(ele).fullCalendar('getDate');
		this.maxTime = (gestorOverWrite) ? "23:59:00":"18:00:00";
	...
	renderedCallback() {
		...
		this.maxTime = (this.gestoroverwrite) ? "23:59:00":"18:00:00";
		this.renderDate = new Date(this.currentdate.toString());
	...
	setTimeHeaderString(start,end,id){
		if(id != 1) {
			start.setHours(start.getHours()-1,start.getMinutes(),'00');
			end.setHours(end.getHours()-1,end.getMinutes(),'00');
		}
		let hoursStart;
		let minutesEnd;
		let hoursEnd
	...
	mapEvents(ev) {
		return ev.map(event => ({
			title: (USER_ID != this.userid)?'Ocupado':event.Subject + ' - ' + event.Account?.Name,

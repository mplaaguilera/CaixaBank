...
export default class Spv_RedaccionEnvioOrganismos extends LightningElement {
    ...
    wiredCasoActual(result) {
        if(result.data){
            this.userId = USER_ID;
            this.desactivarBotonEnviar = (result.data.fields?.OwnerId?.value == this.userId) ? false : true;
            this.organismoCaso = result.data.fields.SPV_Organismo__c.value;
            this.status = result.data.fields.Status.value;  
            this.subestado = result.data.fields.SEG_Subestado__c.value;
            // getDocumentosValidados({'caseId': this.recordId}).then(result =>{
    ...
    enviarOrganismo(){
        ...
        if(this.documentosSeleccionados == null || this.documentosSeleccionados == undefined || this.documentosSeleccionados == '' ){
            ...
            this.isLoading = true;
            enviarDocOrganismos({'caseId': this.recordId, 'enviarReclamante': this.isCheckedEnviarReclamante, 'enviarOrganismos': this.isCheckedEnviarOrganismos, 'organismo': this.organismoCaso, 'estado': this.status, 'subestado': this.subestado, 'listaSeleccionados': this.documentosSeleccionados}).then(()=>{

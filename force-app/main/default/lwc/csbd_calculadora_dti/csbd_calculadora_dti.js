...
import {CAMPOS_OPORTUNIDAD, toast, MENSAJES_INFO, camposCabecera, DATATABLE_TIPOS_BONIFICADOS_COLUMNS, animarCambiosTablas, añadirDatosTitulares} from './utils';
...
export default class csbdCalculadoraDti extends NavigationMixin(LightningElement) {
	...
	componente = {
		...
		funcionesBind: [],
		idTimeouts: {
			abrirTooltipCalculos: null,
			cerrarTooltipCalculos: null,
			abrirPopoverTiposBonificados: null,
			cerrarPopoverTiposBonificados: null,
			abrirPopoverCamposRequeridos: null,
			cerrarPopoverCamposRequeridos: null
		}
	};
	...
	async wiredRecord({data: wiredOpportunity, error: errorGetRecord}) {
		try {
			if (wiredOpportunity) {
				wiredOpportunity = añadirDatosTitulares(wiredOpportunity);
	...
	cardBodyVisible() {
		if (!this.componente.cargado) {
			if (!this.componente.renderInicial && this.data) {
				...
				window.setTimeout(() => {
					this.template.querySelectorAll('.cargando').forEach(e => e.classList.remove('cargando'));
	...
	*/
	...
	popoverTiposBonificadosContainerOnmouseenter({currentTarget: popoverTiposBonificadosContainer}) {
		window.clearTimeout(this.componente.idTimeouts.cerrarPopoverTiposBonificados);
		if (this.datatableTiposBonificados.data.length) {
			const popover = popoverTiposBonificadosContainer.querySelector('section.popoverTiposBonificados');
			this.componente.idTimeouts.abrirPopoverTiposBonificados = window.setTimeout(() => {
	...
	popoverTiposBonificadosContainerOnmouseleave({currentTarget: popoverTiposBonificadosContainer}) {
		window.clearTimeout(this.componente.idTimeouts.abrirPopoverTiposBonificados);
		this.componente.idTimeouts.cerrarPopoverTiposBonificados = window.setTimeout(() => {
			const popoverTiposBonificados = popoverTiposBonificadosContainer.querySelector('section.popoverTiposBonificados');
			if (popoverTiposBonificados.classList.contains('visible')) {
	...
	divTooltipCalculosOnmouseenter({currentTarget: divTooltipCalculos}) {
		window.clearTimeout(this.componente.idTimeouts.cerrarTooltipCalculos);
		const tooltipCalculos = divTooltipCalculos.querySelector('.tooltipCalculos');
		if (!tooltipCalculos.classList.contains('visible')) {
			this.componente.idTimeouts.abrirTooltipCalculos = window.setTimeout(() => tooltipCalculos.classList.add('visible'), 230);
		}
	}

	divTooltipCalculosOnmouseleave({currentTarget: divTooltipCalculos}) {
		window.clearTimeout(this.componente.idTimeouts.abrirTooltipCalculos);
		const tooltipCalculos = divTooltipCalculos.querySelector('.tooltipCalculos');
		if (tooltipCalculos.classList.contains('visible')) {
			this.componente.idTimeouts.cerrarTooltipCalculos = window.setTimeout(() => tooltipCalculos.classList.remove('visible'), 190);
		}
	}

	mensajeInfoOnmouseenter({currentTarget: mensajeInfo}) {
		if (mensajeInfo.dataset.popoverCamposRequeridos) {
			const popoverCamposRequeridos = mensajeInfo.querySelector('div.popoverCamposRequeridos');
			window.clearTimeout(this.componente.idTimeouts.cerrarPopoverCamposRequeridos);
			if (!popoverCamposRequeridos.classList.contains('visible')) {
				this.componente.idTimeouts.abrirPopoverCamposRequeridos = window.setTimeout(() => popoverCamposRequeridos.classList.add('visible'), 230);
			}
		}
	}

	mensajeInfoOnmouseleave({currentTarget: mensajeInfo}) {
		if (mensajeInfo.dataset.popoverCamposRequeridos) {
			window.clearTimeout(this.componente.idTimeouts.abrirPopoverCamposRequeridos);
			const popoverCamposRequeridos = mensajeInfo.querySelector('div.popoverCamposRequeridos');
			if (popoverCamposRequeridos.classList.contains('visible')) {
				this.componente.idTimeouts.cerrarPopoverCamposRequeridos = window.setTimeout(() => popoverCamposRequeridos.classList.remove('visible'), 190);
			}
		}
	}

	pillClienteBotonDetalles() {
		this.template.querySelector('article.cardCalculadoraDti').classList.toggle('mostrarDetallesTitulares');
	}
}
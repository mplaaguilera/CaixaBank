...
export default class Av_ContactBlockCallReport extends LightningElement {
...
get optionsStatus(){
    ...
    handleClickCBOptions(e){
        let start = (this.shownolocalizadocb) ? 1 : 2;
        for(let i = start;i<4 ;i++){
            if(i != e.target.name){
                this.template.querySelector("[data-id='cb"+i+"']").checked = false;
            }
        }
		let optionsMap ={
			'1':{'nolocalizado': e.target.checked},
			'2':{'citanocomercial': e.target.checked},
			'3':{'personal': e.target.checked}
		};
		this.dispatchEvent(new CustomEvent('checkboxinput',{
			detail: optionsMap[e.target.name]
			}	
		));
	...
	seteventcoment(e){	
		this.dispatchEvent(
			new CustomEvent('seteventcoment',
			{
				detail:e.target.value
			})
		);

...
import getAgrupacionesEstrategias from '@salesforce/apex/SIRE_LCMP_HomeGestorPreventivo.getAgrupacionesEstrategias';
...
export default class Sire_lwc_HomeGestorPreventivo extends LightningElement {
    ...
    wiredData(result) {
        ...
        getUsuarioLogeado({usuarioLogeado: USER_ID}).then(resultGetUsuarioLogeado => {          
            this.usuarioLogeadoEsAnalista = resultGetUsuarioLogeado; 

            getAgrupacionesEstrategias({}).then(resultAgrupaciones => {
                this.estrategiasPreventivoCodigo = [];
                ...
                for(let i = 0; i < resultAgrupaciones.length; i++){
                    ...
                    if(resultAgrupaciones[i].Name === 'maxCajas'){
                        ...
                        // Montamos las Estrategias Dinamicas, creando las variables con las estrategias
                        if(!this.estrategiasPreventivoCodigo.includes(resultAgrupaciones[i].SIREC__SIREC_fld_CodigoAgrupador__c)){
                            this.arrayTitulosCajitas.push(resultAgrupaciones[i].SIREC__SIREC_fld_DescAgrupador__c);
                            this.estrategiasPreventivoCodigo.push(resultAgrupaciones[i].SIREC__SIREC_fld_CodigoAgrupador__c);
                        } 
                    }                                   
                }
                ...
                if(result.data){ 
                    ...
                    for(let i = 0; i < result.data.length; i++){        
                        ...
                        if(result.data[i].SIR_fld_Situacion_SF__c === 'SF_INIGEST'){                
                            if(this.estrategiasPreventivoCodigo.includes(result.data[i].SIREC__SIREC_fld_codigoAgrupador__c)){
                                let numArray = this.estrategiasPreventivoCodigo.indexOf(result.data[i].SIREC__SIREC_fld_codigoAgrupador__c);

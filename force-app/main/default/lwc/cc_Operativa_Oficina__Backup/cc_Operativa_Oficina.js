...
export default class ccOperativaOficina extends NavigationMixin(LightningElement) {
	...
	numerosContratos = '';
	Origin;
	@track mostrarModalToastDeudas = false;
	@track mensajeMostrarModalToastDeudas = '';
	...
	*/
				...
				//if(this.autenticacionSAU && nombreTraza != '' && reqAutenticacionSAU.length > 0 && resAutenticacionSAU.length > 0/*reqAutenticacionSAU != '' && resAutenticacionSAU != ''*/){
					//LLamar a Apex para loguear las trazas y pasar el valor que se devuelve en el retorno del get datos
					/*loguearTrazasAutenticacionSAU({
						recordId: this.recordId, 
						nombreTraza: nombreTraza, 
						reqAutenticacionSAU: reqAutenticacionSAU, 
						resAutenticacionSAU: resAutenticacionSAU
					})
					.then()
					.catch(error => {
						console.error(error);

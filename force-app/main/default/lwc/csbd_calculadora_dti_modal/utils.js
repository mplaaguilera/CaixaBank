...
export const MENSAJES_INFO = [
	...
	{
		tipo: 'camposRequeridosNoInformados', mostrar: false, iconName: 'utility:edit', iconoClass: 'iconoError', texto: 'Campos requeridos pendientes de informar.',
		title: 'Importe del inmueble, importe de la hipoteca, interés y plazo.'
	},
...
export function camposCabecera(wiredData) {
	let tipoOperacion = getFieldValue(wiredData, OPPTY_TIPO_OPERACION)?.toUpperCase() ?? '(¿Tipo de operación?)';
	if (tipoOperacion === 'COMPRA DE VIVIENDA') {
		tipoOperacion = 'COMPRAVENTA';
	}
	const usoVivienda = getFieldValue(wiredData, OPPTY_USO_VIVIENDA)?.toUpperCase() ?? '(¿Uso de la vivienda?) ';
	const tipoConstruccion = getFieldValue(wiredData, OPPTY_TIPO_CONSTRUCCION)?.toUpperCase() ?? '(¿Tipo de construcción?)';

	let direccion = '';
	if (getFieldValue(wiredData, OPPTY_VIA_NOMBRE)) {
		if (getFieldValue(wiredData, OPPTY_VIA_TIPO)) {
			direccion += getFieldValue(wiredData, OPPTY_VIA_TIPO);
		}
		if (direccion) {
			direccion += ' ';
		}
		direccion += getFieldValue(wiredData, OPPTY_VIA_NOMBRE);
		if (getFieldValue(wiredData, OPPTY_VIA_NUMERO)) {
			direccion += ' ' + getFieldValue(wiredData, OPPTY_VIA_NUMERO);
		}
		if (getFieldValue(wiredData, OPPTY_VIA_PISO) || getFieldValue(wiredData, OPPTY_VIA_PUERTA)) {
			direccion += ' (';
			if (getFieldValue(wiredData, OPPTY_VIA_PISO)) {
				direccion += getFieldValue(wiredData, OPPTY_VIA_PISO);
			}
			if (getFieldValue(wiredData, OPPTY_VIA_PUERTA)) {
				if (getFieldValue(wiredData, OPPTY_VIA_PISO)) {
					direccion += ' ';
				}
				direccion += getFieldValue(wiredData, OPPTY_VIA_PUERTA);
			}
			direccion += ')';
		}
	}
	const via = direccion;
	if (getFieldValue(wiredData, OPPTY_CODIGO_POSTAL)) {
		if (direccion) {
			direccion += ', ';
		}
		direccion += getFieldValue(wiredData, OPPTY_CODIGO_POSTAL);
	}
	if (getFieldValue(wiredData, OPPTY_MUNICIPIO)) {
		if (direccion) {
			direccion += ' ';
		}
		direccion += getFieldValue(wiredData, OPPTY_MUNICIPIO);
	}
	let comunidadAutonoma = getFieldValue(wiredData, OPPTY_COMUNIDAD_AUTONOMA);
	...
	return {
		...
		mapMarkers: [{
			title: via + ', ' + getFieldValue(wiredData, OPPTY_MUNICIPIO),
			description: '<p>' + tipoOperacion + ' de ' + usoVivienda + '.</p><p>' + tipoConstruccion + '.</p>',
			direccion, icon: 'standard:account', location: {
				value: 'inmueble',
				Country: 'Spain',
				State: getFieldValue(wiredData, OPPTY_COMUNIDAD_AUTONOMA),
				City: getFieldValue(wiredData, OPPTY_MUNICIPIO),
				PostalCode: getFieldValue(wiredData, OPPTY_CODIGO_POSTAL),
				Street: getFieldValue(wiredData, OPPTY_VIA_NOMBRE)
			}
		}],
...
export const DATATABLE_TIPOS_BONIFICADOS_COLUMNS = {
	general: [
		{fieldName: 'tipoConGastos', fixedWidth: 91, cellAttributes: {class: 'gris negrita'}},
		{fieldName: 'importe', fixedWidth: 87, type: 'currency', typeAttributes: {maximumFractionDigits: 0}},
		{type: 'button-icon', fixedWidth: 50, typeAttributes: {name: 'actionAplicar', iconName: 'utility:check', title: 'Aplicar', variant: 'brand'}, cellAttributes: {alignment: 'right'}}
	],
	ccaa: [
		{fieldName: 'tipo', fixedWidth: 45, cellAttributes: {class: 'gris'}},
		{fieldName: 'tipoConGastos', fixedWidth: 46, cellAttributes: {class: 'gris negrita'}},
		{fieldName: 'importe', fixedWidth: 87, type: 'currency', typeAttributes: {maximumFractionDigits: 0}},
		{type: 'button-icon', fixedWidth: 50, typeAttributes: {name: 'actionAplicar', iconName: 'utility:check', title: 'Aplicar', variant: 'brand'}, cellAttributes: {alignment: 'right'}}
	]};
...
export async function animarCambiosTablas(datatableData, datatableDataNew) {
			...
			if (!isNaN(modificaciones[key].anterior) || !isNaN(modificaciones[key].nuevo)) {
				...
				if (ruta[0] === 'calculos') {
					...
					datatablesModificadas.push({
						tabla: ruta[0], fila: ruta[1], aumento: Math.sign(modificaciones[key].nuevo - modificaciones[key].anterior) === 1 ? 'Aumento' : 'Descenso',  //modificaciones[key].anterior <= modificaciones[key].nuevo,
	...
	[...new Set(calculosModificados)].forEach(calculo => {
		...
		if (calculoItem) {
			...
			//eslint-disable-next-line @lwc/lwc/no-async-operation
			window.setTimeout(() => calculoItem.classList.remove('animarCalculo'), 2000);
		}
	});
}

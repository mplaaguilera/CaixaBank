...
import getProcesos from '@salesforce/apex/SIRE_LCMP_HomeGestorFlujo.getQueryProcesos';

export default class Sire_lwc_HomeGestorChart extends LightningElement {
		...
		if (error) {
			...
			data.forEach(proceso => {
				...
				for (var i = 0; i < totalEstrategias.length; i++) { 
					if (proceso.estrategia === totalEstrategias[i]) {
						if(proceso.situacion === totalSituaciones[i]){
							numProcesos++;
						} 
					}
				}

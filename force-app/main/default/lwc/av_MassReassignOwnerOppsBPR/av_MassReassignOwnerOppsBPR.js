...
import fetchData from '@salesforce/apex/AV_MassReassignOwnerOppsBPR_Controller.getBaseData';
...
const columnsOpp = [
	...
	{ label: 'Grupo', fieldName: 'grupo', type:'picklist', hideDefaultActions: true},
    { label: 'Interlocutor', fieldName: 'interlocNameURL', type: 'url', typeAttributes: { label: { fieldName: 'interlocName' } }, hideDefaultActions: true, sortable: true, sortBy:'interlocName'},
	{ label: 'Origen', fieldName: 'origen', type: 'text',hideDefaultActions: true, sortable: true, sortBy:'origen'},
	{ label: 'Producto', fieldName: 'pfName', type: 'text', hideDefaultActions: true},
	{ label: 'Etapa', fieldName: 'stage',type: 'text', hideDefaultActions: true },
	{ label: 'Nombre de la oportunidad', fieldName: 'OppNameURL', type: "url",	typeAttributes:{ label: { fieldName: 'oppName' } }, hideDefaultActions: true, wrapText:true},
	{ label: 'Expectativa de venta', fieldName: 'potencial',type: 'text', hideDefaultActions: true},
	{ label: 'Edad', fieldName: 'age',type: 'number', hideDefaultActions: true},
	{ label: 'Ahorro e Inversión', fieldName: 'ahorro',type: 'currency', hideDefaultActions: true, typeAttributes: {minimumFractionDigits: 0, maximumFractionDigits: 2} },
	{ label: 'Financiacion', fieldName: 'financiacion',type: 'currency', hideDefaultActions: true},
	{ label: 'Ingresos', fieldName: 'ingresos',type: 'currency', hideDefaultActions: true},
	{ label: 'Preconcedido', fieldName: 'preconcedido',type: 'number', hideDefaultActions: true},
	{ label: 'My Box', fieldName: 'myBox', type: 'picklist', hideDefaultActions: true},
	{ label: 'Target Auto', fieldName: 'targetAuto', type:'picklist', hideDefaultActions: true},
	{ label: 'Fecha de vencimiento', fieldName: 'closeDate', type: "date-local",typeAttributes:{ month: "2-digit", day: "2-digit" }},
	{ label: 'Fecha de próxima gestión', fieldName: 'fechaProxRecordatorio',  type: "date-local",typeAttributes:{ month: "2-digit", day: "2-digit" }},
	...
	{ label: 'Oficina', fieldName: 'center', type: 'text', hideDefaultActions: true },
	{ label: 'Indicador Priorizador', fieldName: 'includeInPrioritizingCustomers',type: 'boolean'}
	
];


 
export default class Av_MassReassignOwnerOppsBPR extends LightningElement {
				...
				if(result.recordList != null && result.recordList.length > 0) {
					...
					var rows = result.recordList;
						for (var i = 0; i < rows.length; i++) {
							...
							if(row.pfName != undefined){
								row.pfName = row.pfName;
							} 

							if (row.accountName) {
								row.grupo = row.groupInterlocutor;
								row.interlocName = row.interlocutor;
								row.AccountName = row.accountName;
								row.AccountNameURL = '/' + row.accountId;
								//if (row.Account.AV_Age__c) {
									row.age = row.age;
								//}
								if (row.ahorro) { row.ahorro = row.ahorro; } else { row.ahorro = 0 }
								if (row.financiacion) { row.financiacion = row.financiacion; } else { row.financiacion = 0 }
								if (row.ingresos != null) { row.ingresos = row.ingresos; } else { row.ingresos = 0 }
								if (row.preconcedido) { row.preconcedido = row.preconcedido };
								if (row.myBox) {
									row.myBox = row.myBox;
								}
								if (row.targetAuto) {
									row.targetAuto = row.targetAuto;
								}
							}
	...
	handleSave(){
		...
		if(selected != null && selected.length > 0){
			...
			for(let i=0; i< selected.length ; i++){
				selectedIds.push(selected[i].oppId);
			}
	...
	sortBy(field, reverse, primer) {
		...
		if(field== 'age' || field== 'ahorro' || field== 'financiacion' || field== 'ingresos' ||  field== 'preconcedido' || field == 'AV_IncludeInPrioritizingCustomers__c'){

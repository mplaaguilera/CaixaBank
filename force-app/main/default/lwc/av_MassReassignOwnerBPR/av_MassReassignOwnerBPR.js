...
import fetchData from '@salesforce/apex/AV_MassReassignOwnerBPR_Controller.getBaseData';
...
const columnsTask = [
	...
	{ label: 'Asunto', fieldName: 'SubjectURL', type: 'url', typeAttributes: { label: { fieldName: 'Subject' } }, hideDefaultActions: true},
	...
	{ label: 'Edad', fieldName: 'Av_Age__c', type: 'number', typeAttributes: { label: { fieldName: 'AccountAv_Age' } }, hideDefaultActions: true},
	{ label: 'Ahorro e inversion', fieldName: 'AV_AhorroEInversion__c', type: 'currency', typeAttributes: { length: "16", decimalplaces: "2" }, hideDefaultActions: true},
	{ label: 'Financiación', fieldName: 'AV_Financiacion__c', type: 'currency', typeAttributes: { length: "16", decimalplaces: "2" }, hideDefaultActions: true},
	{ label: 'Ingresos', fieldName: 'AV_Ingresos__c', type: 'currency', typeAttributes: { length: "16", decimalplaces: "2" }, hideDefaultActions: true},//FORMULA
	{ label: 'Precondedido', fieldName: 'AV_Preconceived__c', type: 'currency', hideDefaultActions: true},//FORMULA
					...
					if (this.targetObjName === 'Task') {
						for (var i = 0; i < rows.length; i++) {
							...
							if (row.accountName) {
								...
								row.Av_Age__c = row.age;
								if (row.ahorro) { row.AV_AhorroEInversion__c = row.ahorro; } else { row.AV_AhorroEInversion__c = 0 }
								if (row.financiacion) { row.AV_Financiacion__c = row.financiacion; } else { row.AV_Financiacion__c = 0 }
								if (row.ingresos != null) { row.AV_Ingresos__c = row.ingresos; } else { row.AV_Ingresos__c = 0 }
								if (row.preconcedido) { row.AV_Preconceived__c = row.preconcedido };
								if (row.myBox) {
									row.AV_MyBox__c = row.myBox;
								}
								if (row.targetAuto) {
									row.AV_TargetAuto__c = row.targetAuto;
								}
							}

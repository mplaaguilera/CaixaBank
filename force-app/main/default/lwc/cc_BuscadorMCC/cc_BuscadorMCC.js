...
import buscarResultados from '@salesforce/apex/CC_LCMP_BuscadorMCCController.buscarResultados';
import buscarGruposDetalle from '@salesforce/apex/CC_LCMP_BuscadorMCCController.buscarGruposDetalle';
import actualizarCaso from '@salesforce/apex/CC_LCMP_BuscadorMCCController.actualizarCaso';
import RECTYPE_FIELD from '@salesforce/schema/Case.RecordType.Name';

export default class CC_BuscadorMCC extends LightningElement {
    ...
    @wire(getRecord, {recordId: '$recordId', fields: [RECTYPE_FIELD]})

    wiredCase({data, error}) {
        if (data) {
            console.log('NMA0: '+data);
            console.log('NMA: '+getFieldValue(data, RECTYPE_FIELD));
            this.rt = getFieldValue(data, RECTYPE_FIELD);            
        }else if(error){
            console.error(error);        
        }
    }
    ...
    @wire(buscarResultados, { searchTerm: '$searchInput', tipoBusqueda: '$tipoBuscador', id: '$idSelect', recordTypeName: '$rt'})

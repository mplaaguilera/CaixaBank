...
import responderConsulta from '@salesforce/apex/SPV_LCMP_ResponderConsulta.responderConsulta';
import guardarConsulta from '@salesforce/apex/SPV_LCMP_ResponderConsulta.guardarConsulta';
import getRespuestaConsulta from '@salesforce/apex/SPV_LCMP_ResponderConsulta.getRespuestaConsulta';
import insertarAdjuntoCaso from '@salesforce/apex/SPV_LCMP_ResponderConsulta.insertarAdjuntoCaso';
...
export default class Spv_ResponderConsulta extends LightningElement {
    ...
    get disableResponder(){
        return this.disableResponder;
    }
    ...
    onChangeInput(event){ 
        ...
        this.disableResponder = (inputvalue == '') ? true : false; 
    }

    responder(){
        getRespuestaConsulta({consultaId: this.recordId}).then(result =>{           
            this.valorRespuesta = result;
            if(this.valorRespuesta != '' && this.valorRespuesta != null) {
                this.disableResponder = false;
            }
        })
        .catch(error => {
            const evt = new ShowToastEvent({
                title: 'Fallo al recuperar la respuesta',
    ...
    handleUploadFinished(event) {
        ...
        insertarAdjuntoCaso({consultaId: this.recordId, numFicheros: uploadedFiles.length}).then(() => {
